...
materializes digest-pinned public inputs, JDK 17.0.19, Maven 3.9.11, Rust 1.95.0,
and the source-pinned Kani 0.67.0/CBMC 6.11.0 checkout. It requires Linux amd64.
...
Run Kani only through the checked-in `cmd/kanidriver` evidence workflow. Kani
sequentializes atomics and does not prove overlapping thread schedules. A passing
owner FIFO harness is bounded serialization evidence, not a concurrency proof,
Java equivalence proof, independent review, or release authorization.
