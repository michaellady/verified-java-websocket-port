...
final class OracleEngine {
  ...
  static Map<String, Object> process(String json, String runtimeDigest) throws ProtocolException {
    ...
    try {
      ...
      // MUTANT us005-jm-close-code-1000 (PLANTED, DELIBERATE): reports the
      // NORMAL close code (1000) instead of the library's rejection close
      // code on every InvalidDataException. Expected kill signature:
      // "close_code 1000, expected 1002/1007/1009" on every
      // JAVA_INVALID_DATA error-outcome scenario. See mutants/manifest.json.
      response = execution.failure(new ProtocolException(
          "JAVA_INVALID_DATA", safeRuntimeMessage(e), CloseFrame.NORMAL));
