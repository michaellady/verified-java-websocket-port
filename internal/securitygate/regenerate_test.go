...
func TestUS007RegenerateEvidence(t *testing.T) {
	...
	evidence := validationEvidence{SchemaVersion: policyVersion, Story: "US-007", Company: requiredCompany, Project: requiredProject, PolicyDigests: map[string]string{}, SchemaDigests: map[string]string{}, FixtureCatalogDigest: intake.DigestBytes(read("security/fixtures/cases.json")), AutobahnBaselineDigest: intake.DigestBytes(read("evidence/java/autobahn-baseline.json")), OriginalReceiptDigests: closure.OriginalReceiptDigests, RemediationReceiptDigests: closure.RemediationReceiptDigests, ConsumedRemediationAttemptsPerMode: closure.ConsumedRemediationAttemptsPerMode, FurtherRerunsAuthorized: closure.FurtherRerunsAuthorized, RerunsPerformedByUS007: 0, FixtureResults: []fixtureResult{}, MechanicsState: provenMechanicsState, Assurance: AssuranceOwnerOnly, IndependentReviewClaimed: false, Production: false, Signing: false, Publication: false, SandboxMechanics: provenSandboxMechanics(), SbxLiveEvidence: sbxLiveEvidenceBinding{AttemptID: sbxLiveAttemptID, EvidencePath: sbxLiveEvidencePath, EvidenceDigest: intake.DigestBytes(read(sbxLiveEvidencePath)), ProjectionCanonicalDigest: sbxLiveProjectionCanonicalDigest, TargetCommit: sbxLiveTargetCommit, SourceTree: sbxLiveSourceTree, EnforcementModel: sbxLiveEnforcementModel}, LifecycleIntegration: lifecycleIntegration{State: "BOUND_CANONICAL_VERIFY_REPLAY", EvidenceNodeID: "evidence-security-validation", EvidencePath: "evidence/security-validation.json", LifecyclePath: "assurance/lifecycle.json", Classification: "PUBLIC_DERIVED", VerificationModes: []string{"VERIFY", "REPLAY"}}, Runtime: runtimeMetadata{Provider: "Anthropic", RequestedModel: "claude-fable-5", RequestedReasoningEffort: "high", TaskSessionPath: "us007-claude-plane-evidence-closure", ActualDeploymentIdentifier: "not_exposed", RuntimeSessionUUID: "not_exposed"}}
	for _, path := range policyPaths {
