...
func readArtifact(root, relative string) ([]byte, error) {
	if !safeRelative(relative) {
		return nil, finding("UNSAFE_PATH", relative, "not canonical")
	}
	return readBounded(filepath.Join(root, relative))
}
...
func verifyPlan(root string, plan *Plan) error {
	...
	if plan.MinimumPerRuntime == 0 || plan.MinimumPerRuntime > plan.IdealPerRuntime || plan.InventoryGap == "" || len(plan.Nonclaims) == 0 {
	...
	for index := range plan.Mutants {
		...
		if mutant.Engine != "IN_TREE_PLANTED" || mutant.TimeoutMS < 1000 || mutant.TimeoutMS > 300000 || mutant.Behavior == "" || len(mutant.BuildArgv) == 0 || len(mutant.TestArgv) == 0 || len(mutant.ExpectedKillingTestIDs) == 0 {
...
func verifyMutant(root string, anchor GitAnchor, mutant Mutant, resultDigests map[string]bool) error {
	raw, err := readBounded(filepath.Join(root, mutant.ProductionPath))
	if err != nil {
		return err
	}
...
func verifyRuntimeEvidence(root string, plan Plan, planDigest string, evidence RuntimeEvidence, runtime string) error {
	...
	if !evidence.NoRepositoryDrift || len(evidence.Nonclaims) == 0 || len(evidence.Before) != 2 || len(evidence.After) != 2 {
	...
	source, tests, err := closureDigests(root, runtime)
	...
	for index, result := range evidence.Results {
		...
		for index, observation := range result.Observations {
			...
			if !equalStrings(observation.Build.Argv, mutant.BuildArgv) || !equalStrings(observation.Test.Argv, mutant.TestArgv) {
...
func verifySchemas(root string) error {
	for _, relative := range schemaPaths {
		...
		if strings.Contains(string(raw), "http://") || strings.Contains(string(raw), "https://") {
			return finding("UNSAFE_SCHEMA_REFERENCE", relative, "remote references forbidden")
		}
	}
	return nil
}
...
func closureDigests(root, runtime string) (string, string, error) {
...
func rejectLeakKeys(raw []byte) error {
	var value any
	if err := json.Unmarshal(raw, &value); err != nil {
		return err
	}
