...
// deicticSelfRe finds a record naming its own directory as the population with a
// phrase that can only mean the file's own location.
var deicticSelfRe = regexp.MustCompile(`(?i)in this directory|beside it`)

// deicticReadmeRe is the same idea for "beside this README", which is different
// in one way that matters: it is a claim about the README's directory, and it
// refers to the SOURCE's directory only when the source IS that README.
//
// It bound to the source's own directory unconditionally at first, and the first
// record written after this gate landed tripped it: a review record REPORTING
// the governance README's miscount necessarily quotes the phrase, and the gate
// read `The 62 JSON files beside this README`, quoted from
// evidence/governance/decisions/README.md, as an undispositioned claim about
// drafts/self-review/. maskOtherVoices does not help -- the quotation is inline,
// not a block. Requiring the source to be a README is exact rather than a
// heuristic about quoting, and it costs nothing: no other document can honestly
// make that claim about itself.
var deicticReadmeRe = regexp.MustCompile(`(?i)beside this README`)
...
func censusCandidates(root string, sources []proseSource) (resolvable []claimCandidate, sentences int) {
	for _, source := range sources {
		...
		for i, raw := range lines {
			...
			own := path.Dir(strings.TrimSuffix(source.rel, "#/statement"))
			if deicticSelfRe.MatchString(raw) ||
				(deicticReadmeRe.MatchString(raw) && path.Base(source.rel) == "README.md") {
				cited = append(cited, own)
			}
