...
// necessarily quotes the phrase; the deictic rule bound it to `drafts/self-
// review/` and reported an undispositioned claim against a sentence that says
// nothing about that directory. maskOtherVoices does not catch it — the
// quotation is inline, not a fenced block or a blockquote.
//
// The requirement is exact rather than a heuristic about quoting: only a README
// can honestly say "beside this README" about itself.
func TestAQuotedBesideThisREADMEDoesNotBindTheQuotingRecord(t *testing.T) {
	root := proseRepoRoot(t)
	quoting := proseSource{rel: "drafts/self-review/synthetic.md", kind: "markdown", text: "" +
		"# Synthetic\n\nAlongside it, the store's README says \"The 62 JSON files " +
		"beside this README\" and the count was wrong when written.\n"}
	candidates, sentences := censusCandidates(root, []proseSource{quoting})
	if sentences != 1 {
		t.Fatalf("the sentence must still be COUNTED, only not attributed: sentences=%d", sentences)
	}
	if len(candidates) != 0 {
		t.Fatalf("a record quoting another document's deictic phrase must not be bound "+
			"to its own directory, got %v", candidates)
	}

	// The opposite direction: the README itself still binds, or the fix would
	// have removed the true positive along with the false one.
	readme := proseSource{rel: "evidence/governance/decisions/README.md", kind: "markdown",
		text: "# Owner-decision records\n\nThe 62 JSON files beside this README are the records.\n"}
	candidates, _ = censusCandidates(root, []proseSource{readme})
	if len(candidates) != 1 {
		t.Fatalf("the README's own deictic claim must still be a candidate, got %v", candidates)
	}
