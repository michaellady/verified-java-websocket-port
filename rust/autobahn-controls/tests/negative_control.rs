...
fn peer_bounds() -> IoBounds {
    ...
    IoBounds {
        ...
        // 8s, the window the old `max_polls: 4_000` bought at this
        // read_timeout, and far longer than the control's 150ms linger so the
        // control is still always the side that hangs up first.
        max_polls: polls_for(Duration::from_secs(8), read_timeout),
        ..IoBounds::default()
    }
}
