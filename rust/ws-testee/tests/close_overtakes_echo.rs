...
/// The fixture's liveness bound, stated as a DURATION and converted, not
/// chosen as a magnitude.
///
/// This mirrors `loopback.rs`'s `polls_for` deliberately rather than sharing
/// it: these are separate test binaries, and the duplication is three lines
/// against a cross-file test module. The reasoning is the same one F005 fixed
/// and `make -C rust fixture-guard` now enforces as shape C — `IoBounds` gives
/// the shared I/O loop only a count, so a fixture that means "don't hang" must
/// say how long and let the count follow. The number this replaced,
/// `max_polls: 2_000_000`, is F005's own losing magnitude, and it reached this
/// file because this branch was written in parallel with the detector that
/// catches it: alone each was green, and only the merge was red.
...
fn prompt_bounds() -> IoBounds {
    ...
    IoBounds {
        read_timeout,
        max_polls: polls_for(Duration::from_secs(30), read_timeout),
        ..IoBounds::default()
    }
}
