...
## Merge addendum — two defects found landing this branch

Added by the integrator at merge, not by the author of the work above. Both were
found by running the gate rather than by reading the record, and neither changes
a finding or a number in it.

### The gate's first customer tripped it

`record-guard prose` failed on the merged tree with one blocking finding, and it
was against the record REPORTING this work:

```
UNDISPOSITIONED_CLAIM record=drafts/self-review/plan-node-deletion.md
  field="drafts/self-review/" line=152
  detail="this line states a cardinality about a population this gate can
  enumerate ... | Alongside it, the store's README says "The 62 JSON files
  beside this README" and
```

`censusCandidates` matched `deicticRe` on "beside this README" and appended the
SOURCE's own directory, so a record quoting the governance README's phrase was
read as making a claim about `drafts/self-review/`. It says nothing about that
directory. `maskOtherVoices` does not reach it: the quotation is inline, not a
fenced block or a blockquote.

The class is worth naming. This gate's first real customer was a record
reporting the README's miscount, and reporting it *necessarily* quotes the
phrase. A gate that fails the record that reports its own finding is one people
route around.

Fixed by splitting the pattern: `deicticSelfRe` ("in this directory", "beside
it") applies to any source, and `deicticReadmeRe` ("beside this README") applies
only when the source IS a `README.md`. That is exact rather than a heuristic
about quoting — no other document can honestly make that claim about itself —
and it costs nothing, because a record that counts its own directory in the
deictic form the first pattern matches is still caught.

Verified on the real tree, both directions:

| tree | result |
| --- | --- |
| unmodified | `cardinality_sentences=247 with_enumerable_population=11 bound=11 undispositioned=0`, PASS |
| README corrected to 63 | `STALE_ALLOWANCE ... "allowed at stated value \"62\", but that claim no longer disagrees with the document"` |
| README wrong differently (61) | `PROSE_DISAGREES_WITH_DOCUMENT ... "the record says 61, the tree derives 63 from evidence/governance/decisions/*.json"` |
| restored | PASS |

`TestAQuotedBesideThisREADMEDoesNotBindTheQuotingRecord` pins both: the quoting
record yields zero candidates while its sentence is still COUNTED, and the README
itself still yields one with population `evidence/governance/decisions/`.

### The test helper reached back into the checkout

Gates then failed `go-suite` in two packages this branch does not touch:
`cmd/securityctl` and `internal/assurance`. Both passed in isolation. The easy
reading was contention between the five worktrees running at the time, and it was
wrong.

`mirrorTree` hard-linked every file of the checkout into its temp mirror. A hard
link raises `st_nlink` on the SOURCE, and `internal/assurance` requires
`assurance/lifecycle.json` to be "an immutable single-link file"
(`adapter.go:1213`, `beforeIdentity.nlink != 1`). The mirror's skip set is only
`.git`, `.quarantine`, `target` and `out`, so that file was mirrored. `go test
./...` runs packages concurrently. While a mirror was alive in this package, that
assertion failed in the other one.

The helper's stated reason for hard links was to run the real rule over the REAL
records rather than a fixture. A byte copy keeps that property exactly; the links
were a speed choice, and 36 MB over 3014 files copies in under a second.

Sampling would not have settled it — four green runs against a failure seen once
in three is weak evidence — so the mechanism is asserted directly by
`TestMirroringDoesNotDisturbTheCheckout`, and the test was shown to FAIL on the
old behaviour before being trusted:

```
--- FAIL: TestMirroringDoesNotDisturbTheCheckout
    mirroring raised the checkout's link count on
    /home/user/vjwp-main/assurance/lifecycle.json to 2;
    internal/assurance requires it to stay 1
```

A mirror whose whole purpose is to keep mutations out of the checkout should not
have been reaching into it in the first place.
